...
void train( TString myMethodList = "", TString outfileName = "TMVAReg.root" ) {
...
//    vector<vector<int> > *matchedESRecHit_plane;
   ...
   std::vector<int> notToTrain = {20, 21, 48, 228, 245, 271, 343, 352, 376, 456, 554, 592};
   
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) { 
       TString name; name.Form ("ES_plane1[%d]", ixtal);
   ...
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) { 
       TString name; name.Form ("ES_plane2[%d]", ixtal);
       dataloader->AddVariable( name.Data(), name.Data(), "GeV", 'F' );
     }
   }
   ...
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) { 
       TString name; name.Form ("HCAL_depth1[%d]", ixtal);
       dataloader->AddVariable( name.Data(), name.Data(), "GeV", 'F' );
     }
   }
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) { 
       TString name; name.Form ("HCAL_depth2[%d]", ixtal);
       dataloader->AddVariable( name.Data(), name.Data(), "GeV", 'F' );
     }
   }
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) { 
       TString name; name.Form ("HCAL_depth3[%d]", ixtal);
       dataloader->AddVariable( name.Data(), name.Data(), "GeV", 'F' );
     }
   }
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) { 
       TString name; name.Form ("HCAL_depth4[%d]", ixtal);
       dataloader->AddVariable( name.Data(), name.Data(), "GeV", 'F' );
     }
   }
...
//    dataloader->AddSpectator( "spec2:=var1*3",  "Spectator 2", "units", 'F' );
   ...
   for (int ixtal=0; ixtal<nxtals; ixtal++) {
     auto valuefound = std::find(notToTrain.begin(), notToTrain.end(), ixtal);
     if(valuefound == notToTrain.end()) {   
       TString name; name.Form ("ECAL[%d]", ixtal);
       dataloader->AddTarget( name.Data() );
     }
   }
...
//    TTree *regTree = (TTree*)input->Get("rechitdumper/caloTree");
   TTree *regTree = (TTree*)input->Get("mytree");
